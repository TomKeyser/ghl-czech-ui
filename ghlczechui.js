...
(function () {
  ...
  var DICT = {
    ...
    /* ===================================================================
       MEDIA STORAGE + REPUTATION + REPORTING -- deep pass (v25).
       NOT COVERABLE: Reporty > Local Marketing Audit renders inside a
       cross-origin iframe at prod.analyzemy.business.
       =================================================================== */
    ...
    /* --- == REPUTATION: requests / reviews / video == --- */
    ...
    /* CASE GUARD: adding 'On' means the case-insensitive fallback would also
       catch the lowercase word "on" that HighLevel renders as its own text
       node inside "...will end at 02:18 on Saturday..." (countdown timer).
       All three casings are therefore mapped explicitly so no reading of the
       word can fall through to the wrong one. */
    'on': 'dne',
    'ON': 'ZAPNUTO',
    'Off': 'Vypnuto',
    'On': 'Zapnuto',
  ...
  */
  ...
  /* "0 Bytes used" -- the media-storage quota chip */
  var BYTES_USED  = /^([\d.,]+)\s*(Bytes|B|KB|MB|GB|TB)\s+used$/i;
  /* "0 Accounts" -- review-platform connection chips */
  var N_ACCOUNTS  = /^(\d+)\s+Accounts?$/i;
  /* "Aug 19, 2026 - Sep 3, 2026" -- the ads date-range button */
  var DATE_RANGE  = /^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+(\d{1,2}),\s*(\d{4})\s*-\s*(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+(\d{1,2}),\s*(\d{4})$/;
  ...
  function translate(raw) {
    ...
    var b1 = BYTES_USED.exec(key);
    if (b1) return 'Využito ' + b1[1] + ' ' + b1[2];

    var b2 = N_ACCOUNTS.exec(key);
    if (b2) return b2[1] + ' ' +
      czPlural(parseInt(b2[1], 10), 'účet', 'účty', 'účtů');

    var b3 = DATE_RANGE.exec(key);
    if (b3) return b3[2] + '. ' + MONTH_ABBR[b3[1].toLowerCase()] + ' ' + b3[3] +
      ' – ' + b3[5] + '. ' + MONTH_ABBR[b3[4].toLowerCase()] + ' ' + b3[6];
