  ...
  */
  ...
  var CONTENT_ZONES = [
    ...
    /* AND THE REST OF THAT FAMILY, v98. The same widget row has pickers for
       ...
       selector reaches them; the record backstop is what covers those (see
       FIREWALL.md, "Known gaps"). */
  ...
  /* a viewport-sized frame we cannot read IS the wall. One definition, shared
     by frames() and the notices, so the diagnostic and the user-facing
     sentence can never disagree about which screens are walled. */
  ...
  /* [border, fill, text]. 'limit' STANDS OUT on purpose — Tom, 13 Sep: "the same
     colour as the Claude logo, and make the text bold". Fill #D97757 with
     near-black text: white on that orange is about 3.1:1, below the 4.5:1 that
     14 px text needs, and #1a1a1a is about 5.6:1. A colour names nobody, so the
  ...
  /* storage can throw or come back empty. Then the notice SHOWS: a repeated
  ...
  function scheduleNotices() {
    ...
    noticeTimer = window.setTimeout(function () {
      noticeTimer = null;
      noticeLast = Date.now();
      try { updateNotices(); } catch (e) { /* never break the app */ }
  ...
  function flush() {
    ...
    /* notices follow the screen: re-anchored when a frame is replaced, removed
       when the gate closes. Nothing is scheduled while they are off. */
