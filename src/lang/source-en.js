...
(function (root) {
  ...
  var source = {
    ...
    rules: [
      ...
      /* NO RULE FOR "By <developer>" on the marketplace cards — REMOVED in v131.
         v130 added one, and it never fired: the whole card header is a content
         zone since v81 (the developer's own words), so the text never reaches a
         rule. __kaDebug.why(element) says so in one call. Ask why() BEFORE writing
         a rule for text that looks untranslated. */
      ['N_NEW',          /^(\d+)\s+new$/i],
